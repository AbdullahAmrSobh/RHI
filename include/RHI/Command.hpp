...
public:
    ...
    // Binds a list of ShaderResourceGroups
    virtual void SetShaderResourceGroups(std::span<const ShaderResourceGroup*> shaderResourceGroups) = 0;

    // Submits a command for rendering
    virtual void Submit(const Draw& cmd) = 0;

    // Submits a command for dispatching
    virtual void Submit(const Compute& cmd) = 0;

    // Submits a command for transfer
    virtual void Submit(const Copy& cmd) = 0;
};
