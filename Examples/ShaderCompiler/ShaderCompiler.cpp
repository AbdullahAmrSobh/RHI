...
TL::Result<ShaderFile> ShaderCompiler::CompileShader(const char* path)
{
    ShaderFile shaderFile;

    // Read shader source from file using TL::File
    TL::File file;
    ...
    if (file.read(source.data(), size).result != TL::IOResultCode::Success)
    {
    ...
    auto                    result  = m_session->createCompileRequest(&request);
    if (SLANG_FAILED(result) || !request)
    {
        return TL::Error("Failed to create Slang compile request");
    }

    // Add the shader source as a translation unit
    int translationUnitIndex = request->addTranslationUnit(SLANG_SOURCE_LANGUAGE_SLANG, nullptr);
    request->addTranslationUnitSourceString(translationUnitIndex, path, source.c_str());

    // Add entry points (for now, just try "main")
    int entryPointIndex = request->addEntryPoint(translationUnitIndex, "main", SLANG_STAGE_COMPUTE);
    ...
    if (SLANG_FAILED(result))
    {
        ...
        // request->getDiagnosticOutput(diagnosticsBlob.writeRef());
        // LogDiagnostics(diagnosticsBlob);
        // request->release();
        // return std::nullopt;
    }

    // Get reflection and bytecode
    Slang::ComPtr<slang::IComponentType> program;
    if (SLANG_SUCCEEDED(request->getProgram(program.writeRef())))
    {
        if (auto programLayout = program->getLayout())
        {
            ParseShaderParameterBlock(programLayout);
        }
    }

    // Get compiled code (SPIR-V as example)
    slang::IBlob* codeBlob = nullptr;
    result                 = request->getEntryPointCodeBlob(entryPointIndex, 1, &codeBlob);
    if (SLANG_SUCCEEDED(result) && codeBlob)
    {
        // shaderFile.bytecodeData.resize(codeBlob->getBufferSize());
        // memcpy(shaderFile.bytecodeData.data(), codeBlob->getBufferPointer(), codeBlob->getBufferSize());
    }
    else
    {
        request->release();
