...
#ifdef RHI_LINUX
Expected<Unique<ISurface>> Instance::CreateSurface(
    const X11SurfaceDesc& desc) const
{
...
Expected<Unique<ISwapchain>> Device::CreateSwapChain(
...
VkExtent2D Surface::ClampExtent(VkExtent2D actualExtent,
...
VkResult Swapchain::Init(const SwapchainDesc& desc)
{
    ...
    VkSurfaceCapabilitiesKHR surfaceCaps =
        surface.GetCapabilities(m_pDevice->GetPhysicalDevice());
    std::vector<VkPresentModeKHR> availablePresentModes =
        surface.GetSupportedPresentModes(m_pDevice->GetPhysicalDevice());
    std::vector<VkSurfaceFormatKHR> availableSurfaceFormats =
        surface.GetSupportedFormats(m_pDevice->GetPhysicalDevice());

    VkSurfaceFormatKHR selectedFormat =
        Surface::SelectFormat(availableSurfaceFormats);
