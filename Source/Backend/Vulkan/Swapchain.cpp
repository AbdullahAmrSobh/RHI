...
namespace Vulkan
{
    ...
    {
        Unique<ShaderModule> surface = CreateUnique<ShaderModule>(*this);
    ...
    VkResult Surface::Init(const Win32SurfaceDesc& desc)
    {
        ...
        static PFN_vkCreateWin32SurfaceKHR createSurface = (PFN_vkCreateWin32SurfaceKHR)vkGetInstanceProcAddr(m_pInstance->GetHandle(), "vkCreateWin32SurfaceKHR");
    ...
    Swapchain::~Swapchain()
    {
        for(auto image : m_backBuffers) delete image;
    ...
    VkResult Swapchain::InitBackbuffers()
    {
        ...
        {
            m_backBuffers.push_back(new Image(*m_pDevice, imageHandle));
        }
