...
void CommandBuffer::SetViewports(const std::vector<Viewport>& viewports)
{
    ...
    std::transform(viewports.begin(), viewports.end(),
                   std::back_inserter(vkViewports),
                   [](Viewport viewport) { return ConvertViewport(viewport); });
...
void CommandBuffer::SetScissors(const std::vector<Rect>& scissors)
{
    ...
    std::transform(scissors.begin(), scissors.end(),
                   std::back_inserter(scissorRects),
                   [](Rect rect) { return ConvertRect(rect); });
...
void CommandBuffer::Submit(const DrawCommand& drawCommand)
{
    ...
    const PipelineState& pso =
        *static_cast<const PipelineState*>(drawCommand.pPipelineState);
    ...
    vkCmdBindDescriptorSets(
        m_handle, VK_PIPELINE_BIND_POINT_GRAPHICS, pso.GetLayout().GetHandle(),
        0, CountElements(descriptorSets), descriptorSets.data(), 0, nullptr);
...
{
    if (copyCommand.destinationResourceType == EResourceType::Buffer
        && copyCommand.sourceResourceType == EResourceType::Buffer)
    {
        Buffer& srcBuffer =
            static_cast<Buffer&>(*copyCommand.srcBuffer.pBuffer);
        Buffer& dstBuffer =
            static_cast<Buffer&>(*copyCommand.dstBuffer.pBuffer);
    ...
    else if (copyCommand.destinationResourceType == EResourceType::Buffer
             && copyCommand.sourceResourceType == EResourceType::Image)
    {
        VkCopyImageToBufferInfo2 copyInfo;
        vkCmdCopyImageToBuffer2(m_handle, &copyInfo);
    }
    else if (copyCommand.destinationResourceType == EResourceType::Image
             && copyCommand.sourceResourceType == EResourceType::Image)
    {
...
void CommandBuffer ::BeginRenderPass(Extent2D                  extent,
...
VkResult CommandAllocator::Init(ECommandPrimaryTask task)
{
    ...
    return vkCreateCommandPool(m_pDevice->GetHandle(), &createInfo, nullptr,
                               &m_handle);
}
...
std::vector<CommandBuffer> CommandAllocator::AllocateCommandBuffers(
    uint32_t count)
{
    VkCommandBufferAllocateInfo allocateInfo = {};
    allocateInfo.sType       = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    ...
    vkAllocateCommandBuffers(m_pDevice->GetHandle(), &allocateInfo,
                             handles.data());
