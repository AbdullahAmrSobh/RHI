...
namespace Vulkan
{
    ...
    Pass::~Pass()
    {
        m_context->FreeSemaphore(m_signalSemaphore);
    }
    ...
    void FrameScheduler::OnBegin()
    {

    }

    void FrameScheduler::OnEnd()
    {
    ...
    void FrameScheduler::QueuePassSubmit(RHI::Pass* _pass, RHI::Fence* _signalFence)
    {
        auto context = (Context*)m_context;
        auto pass = (Pass*)_pass;
        auto fence = (Fence*)_signalFence;
        auto queue = context->GetQueue(pass->m_queueType);

        auto signalSemaphore = GetPassSignalSemaphoreSubmitInfo(pass);
    ...
    void FrameScheduler::QueueImagePresent(RHI::ImageAttachment* attachment, RHI::Fence* _signalFence)
    {
        auto context = (Context*)m_context;
        auto fence = (Fence*)_signalFence;
    ...
    void FrameScheduler::QueueCommandsSubmit(RHI::QueueType queueType, RHI::TL::Span<RHI::CommandList*> _commandLists, RHI::Fence* _signalFence)
    {
        auto context = (Context*)m_context;
        auto fence = (Fence*)_signalFence;
    ...
    VkSemaphoreSubmitInfo FrameScheduler::GetPassSignalSemaphoreSubmitInfo(Pass* pass)
    {
